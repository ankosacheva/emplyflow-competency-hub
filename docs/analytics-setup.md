...
Внизу экрана появляется типичный cookies-бар:

- **Принять** → `localStorage.efhub_consent=granted`, события отправляются
- До согласия события не уходят

`visitor_id` создаётся локально и после согласия / в заявке позволяет связать путь.
